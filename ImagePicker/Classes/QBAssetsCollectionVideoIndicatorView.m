...
@interface QBAssetsCollectionVideoIndicatorView()
...
//    NSString *durationString = [NSString stringWithFormat:@"%02ld:%02ld", (long)minutes, (long)seconds];
    ...
    if(0 <= self.duration && self.duration < 60 * 60) {
        NSInteger min = (NSInteger)(self.duration / 60.0);
        NSInteger sec = (NSInteger)ceil(self.duration - 60.0 * (double)min);
        durationString = [NSString stringWithFormat:@"%ld:%02ld", min, sec];
    } else if(60 * 60 <= self.duration && self.duration < 60 * 60 * 60) {
        NSInteger hour = (NSInteger)(self.duration / (60.0 * 60.0));
        NSInteger min = (NSInteger)((self.duration - (60.0 * 60.0) * hour) / 60.0);
        NSInteger sec = (NSInteger)ceil(self.duration - (60.0 * 60.0) * hour - (60 * min));
        durationString = [NSString stringWithFormat:@"%ld:%02ld:%02ld", hour, min, sec];
    }
    
    if (![durationString respondsToSelector:@selector(drawInRect:withAttributes:)]) {
